...
CHURN_P_XGB <- as.numeric(CHURN_Rank_XGB > 0.50) # make prediction binary for cf matrix
...
CHURN_P_Train <- as.numeric(CHURN_Rank_Train > 0.50) # binary prediction for test
...
print(test_riskchart) # chart to understand how the predictions impact operational caseloads
...
k_test <- m_test_id %>% dplyr::select(CUSTOMER, CHURN, CHANGEM, MOU, CHANGER, DROPBLK, MOUREC) %>% na.omit()
...
cluster_train <- cbind(k_train, CUST_Clusters_Train) # training dataset with clusters
...
CHURN_P <- as.numeric(CHURN_Rank > 0.50) # make aggregated prediction binary
...
test_riskchart <- riskchart(ge_cust_churn_df$CHURN_Rank,
